...
class AppointmentServiceTest {
	...
	void testUpdateAppointment() {
		as.addAppointment(new Appointment("id", new Date(200, 10, 26), "description"));
		as.updateAppointment("id", new Date(201, 11, 27), "description2");
		Appointment a = as.getAppointmentList().get(0);
		
		assertEquals(as.getAppointmentList().size(), 1);
		assertEquals(a.getAppointmentID(), "id");
		assertEquals(a.getDescription(), "description2");
	}
	
	@Test
	void testUpdateAppointmentNoElementsInList() {
		Assertions.assertThrows(IllegalArgumentException.class, () -> {
			as.updateAppointment("id", new Date(200, 10, 26), "description");
		});
	}
	
	@Test
	void testUpdateAppointmentBadAppointmentID() {
		as.addAppointment(new Appointment("id", new Date(200, 10, 26), "description"));
		
		Assertions.assertThrows(IllegalArgumentException.class, () -> {
			as.updateAppointment("id2", new Date(201, 11, 27), "description2");
		});
	}
	
	@Test
	void testUpdateAppointmentAppointmentIDNull() {
		as.addAppointment(new Appointment("id", new Date(200, 10, 26), "description"));
		
		Assertions.assertThrows(IllegalArgumentException.class, () -> {
			as.updateAppointment(null, new Date(201, 11, 27), "description2");
		});
	}
